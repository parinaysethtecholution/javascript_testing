...
const logout = async (req, res) => { // This function was added in the commit
    try {
        ...
        res.status(200).json({ message: 'Logout successful' }); // Sends a success message upon successful logout
    } catch (error) {
        console.error('Error in logout:', error); // Logs any error that occurs during logout
        res.status(500).json({ error: 'Internal Server Error' }); // Sends an error message if an error occurs
    }
};
...
module.exports = {
    register,
    login,
    logout // This was added in the commit
};
